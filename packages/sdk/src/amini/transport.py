...
class HttpTransport:
    """Synchronous transport — suitable for threaded flush loops."""
...
class AsyncHttpTransport:
    """Async transport — avoids blocking the event loop in async applications.

    Use this when instrumenting async agents (e.g. LangChain, CrewAI) to
    prevent synchronous HTTP calls and ``time.sleep()`` from blocking the
    event loop.
    """
    ...
    async def send_batch(self, events: list[Event]) -> bool:
        if not events:
            return True
        ...
        for attempt in range(MAX_RETRIES + 1):
            try:
                response = await self._client.post("/api/v1/events/batch", json=payload)
                response.raise_for_status()
                return True
            except httpx.HTTPError as e:
                if attempt < MAX_RETRIES:
